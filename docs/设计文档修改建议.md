...
**生成时间**: 2026-01-XX  
**文档版本**: System Design v2.0  
**审查范围**: 系统设计文档、产品需求文档、数据库Schema
...
**问题位置**: `docs/system_design_document.md` Section 6.3

**问题描述**:
- **Step 1** 说: "Total Charge: $294.50" (包含 $200 押金)
- **Step 2** 说: "Deposit: **Authorized Hold** (Stripe) - frozen on card, not charged"

这是矛盾的：如果押金是"预授权冻结"（不扣款），那么 Total Charge 不应该包含押金。
...
**问题描述**:
1. 设计文档定义了6种交易模型，每种都有不同的状态机
2. 但代码中有两个不同的 `OrderStatus` 定义：
   - `src/types/domain.ts`: 基础状态（6个）
   - `src/types/orders.ts`: 扩展状态（10个）
3. 设计文档中的状态（如 `CONFIRMED_VISIT`, `VISIT_COMPLETED`, `PICKED_UP`, `RETURNED`, `SETTLED`）在代码中缺失

**修改建议**:

#### 2.1 统一 OrderStatus 类型定义

在 `src/types/orders.ts` 中扩展状态以覆盖所有交易模型：
...
在 Section 4.7 中添加状态转换图，明确每个状态的触发条件：
...
DRAFT 
  → (User pays visit fee) 
PENDING_DEPOSIT 
  → (Provider confirms visit) 
CONFIRMED_VISIT 
  → (Provider completes visit) 
VISIT_COMPLETED 
  → (Provider submits quote via chat) 
PENDING_QUOTE_PAYMENT 
  → (User accepts quote & pays) 
ACCEPTED_QUOTE 
  → (Work starts) 
IN_PROGRESS 
  → (Work completed) 
...
**问题描述**:
设计文档 Section 6.4 要求添加租赁相关字段，但 `supabase_schema.sql` 中的 `orders` 表缺少：
...
在 `docs/supabase_schema.sql` 的 `orders` 表定义后添加：
...
**问题描述**:
设计文档定义了三级优先级选择逻辑（Item > Master > Category），但代码中：
- `listing_masters` 表缺少 `preferred_transaction_model` 字段
- `listing_items` 表缺少 `transaction_model_override` 字段
- 没有实现选择逻辑函数
...
在 `src/services/repositories/supabase/OrderRepository.ts` 中添加：
...
**问题描述**:
设计文档详细说明了加拿大税务规则，但：
1. `provider_profiles` 表缺少 `is_tax_registered` 字段
2. 缺少税务计算函数
3. 前端没有显示税务明细
...
创建 `src/services/taxCalculator.ts`:
...
**问题描述**:
设计文档要求快照包含完整的 ListingMaster 和 ListingItem 状态，但当前的 `OrderSnapshot` 缺少：
- 服务商位置信息
- 服务范围（radius）
- 认证级别
- 许可证信息
...
**问题描述**:
设计文档定义了 "Quote-via-Chat" 流程，但：
1. `messages` 表的 `message_type` 只有 'TEXT'，缺少 'QUOTE'
2. 没有实现 Quote 消息的数据结构
3. 没有实现 "Accept Quote" 按钮逻辑
...
在 Section 23.3 中添加 Quote 消息的数据结构示例。
...
**建议**: 在系统设计文档中添加专门的 "Error Handling & Edge Cases" 章节，包括：
- 支付失败处理
- 网络中断恢复
- 并发订单冲突
- 库存不足处理
...
**建议**: 在系统设计文档中添加 "Performance & Scalability" 章节，定义：
- API 响应时间目标（< 200ms）
- 数据库查询优化策略
- 缓存策略（Redis/内存缓存）
- 图片CDN使用
...
**建议**: 创建 `docs/TESTING_STRATEGY.md`，包括：
- 单元测试覆盖率目标（> 80%）
- 集成测试场景
- E2E测试流程
- 负载测试计划
...
**建议**: 在系统设计文档中添加 "Database Migration Strategy" 章节，包括：
- Schema版本管理
- 数据迁移脚本规范
- 回滚策略
- 零停机迁移方案
...
**建议**: 创建 `docs/API_DOCUMENTATION.md`，包括：
- Repository接口规范
- 错误码定义
- 请求/响应格式
- 认证机制
...
| 优先级 | 问题 | 影响 | 建议完成时间 |
|--------|------|------|-------------|
| 🔴 P0 | 租赁押金逻辑矛盾 | 支付流程错误 | 立即修复 |
| 🔴 P0 | 订单状态机不完整 | 业务流程阻塞 | 1周内 |
| 🔴 P0 | 数据库Schema缺失 | 功能无法实现 | 1周内 |
| 🔴 P0 | 交易模型选择逻辑 | 业务逻辑错误 | 2周内 |
| 🟡 P1 | 税务计算规则 | 合规风险 | 2周内 |
| 🟡 P1 | 订单快照结构 | 数据完整性 | 2周内 |
| 🟡 P1 | Quote消息类型 | 功能缺失 | 3周内 |
| 🟢 P2 | 错误处理文档 | 开发效率 | 4周内 |
| 🟢 P2 | 性能指标定义 | 可扩展性 | 4周内 |
...
- [ ] 所有数据库Schema变更已应用到 `supabase_schema.sql`
- [ ] TypeScript类型定义已更新并同步
- [ ] 设计文档中的代码示例已验证可执行
- [ ] 所有状态转换图已更新
- [ ] 前端组件已适配新的状态和字段
- [ ] 测试用例已更新覆盖新逻辑
